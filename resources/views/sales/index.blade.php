    ...
    <div class="card-body">
            ...
            <div class="col-12 col-md-3">
                <div class="card dashboard-card text-white border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #16a34a, #22c55e);">
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            <h3 class="mb-1 text-white">{{ $completedSales ?? $sales->where('status', 'concluida')->count() }}</h3>
            ...
            <div class="col-12 col-md-3">
                <div class="card dashboard-card text-white border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #f59e0b, #eab308);">
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            <h3 class="mb-1 text-white">{{ $pendingSales ?? $sales->where('status', 'pendente')->count() }}</h3>
            ...
            <div class="col-12 col-md-3">
                <div class="card dashboard-card text-white border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #d97706, #f59e0b);">
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            <h3 class="mb-1">R$ {{ number_format($salesRevenue ?? $sales->sum('total'), 2, ',', '.') }}</h3>
        ...
        <form method="GET" action="{{ route('sales.index') }}" class="row g-3 mb-4">
            <div class="col-12 col-md-4">
                ...
                <input type="text" id="search" name="search" class="form-control bg-dark text-white border-secondary" value="{{ request('search') }}" placeholder="Digite o nome do cliente">
            </div>
                ...
                <tbody>
                    @forelse ($sales as $sale)
                        <tr>
                            ...
                            <td>{{ $sale->sale_date ? $sale->sale_date->timezone(config('app.timezone'))->format('d/m/Y H:i') : '-' }}</td>
                            <td>
                                @php
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-2 flex-wrap">
                                    ...
                                    <form action="{{ route('sales.destroy', $sale) }}" method="POST" onsubmit="return confirm('Excluir esta venda?')">
                                        @csrf
                                        @method('DELETE')
