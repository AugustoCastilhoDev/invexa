...
<head>
    ...
    <style>
        ...
        .sa-section-title {
            ...
            letter-spacing: .1em; color: var(--sky); margin-bottom: .75rem;
        }
        ...
        .sa-table thead th {
            ...
            letter-spacing: .08em; color: rgba(148,163,184,.5);
            padding: .6rem 1rem; border-bottom: 1px solid rgba(14,165,233,.1);
        ...
        /* BOTÃO TOGGLE */
        .btn-toggle-on  { background: rgba(239,68,68,.12); border: 1px solid rgba(239,68,68,.25); color: #f87171; font-size: .75rem; padding: .25rem .75rem; border-radius: .4rem; cursor: pointer; transition: background .2s; }
        .btn-toggle-on:hover  { background: rgba(239,68,68,.22); }
        .btn-toggle-off { background: rgba(34,197,94,.12);  border: 1px solid rgba(34,197,94,.25);  color: #4ade80; font-size: .75rem; padding: .25rem .75rem; border-radius: .4rem; cursor: pointer; transition: background .2s; }
        ...
        /* PLAN DIST BAR */
        .plan-bar-wrap { display: flex; gap: .5rem; align-items: center; font-size: .78rem; }
        .plan-bar { height: 6px; border-radius: 999px; }
    </style>
</head>
<body>
    ...
    <div class="row g-3 mb-4">
                ...
                @foreach(['free', 'pro', 'business'] as $plan)
                    ...
                    <div class="mb-3">
                        ...
                        <div style="height:6px; background:rgba(255,255,255,.06); border-radius:999px; overflow:hidden;">
                            <div style="height:100%; width:{{ $pct }}%; background:{{ $planColors[$plan] }}; border-radius:999px; transition:width .6s;"></div>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="col-12 col-lg-8">
            <div class="metric-card h-100">
                ...
                <div class="row g-3">
                    <div class="col-6">
                        <div style="font-size:.75rem; color:rgba(148,163,184,.55);">Empresas Ativas</div>
                        <div style="font-size:1.5rem; font-weight:800; color:#4ade80;">{{ $activeCompanies }}</div>
                    </div>
                    <div class="col-6">
                        <div style="font-size:.75rem; color:rgba(148,163,184,.55);">Empresas Inativas</div>
                        <div style="font-size:1.5rem; font-weight:800; color:#f87171;">{{ $totalCompanies - $activeCompanies }}</div>
                    </div>
                    <div class="col-6">
                        <div style="font-size:.75rem; color:rgba(148,163,184,.55);">Plano Pro</div>
                        <div style="font-size:1.5rem; font-weight:800; color:#38BDF8;">{{ $planCounts['pro'] ?? 0 }}</div>
                    </div>
                    <div class="col-6">
                        <div style="font-size:.75rem; color:rgba(148,163,184,.55);">Plano Business</div>
                        <div style="font-size:1.5rem; font-weight:800; color:#c084fc;">{{ $planCounts['business'] ?? 0 }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <table class="sa-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span class="plan-badge plan-{{ $company->plan }}">{{ $company->plan }}</span>
                    </td>
                    <td>
                    ...
                    <td>
                        <form action="{{ route('admin.companies.toggle', $company) }}" method="POST" class="m-0">
                            @csrf @method('PATCH')
                            <button type="submit" class="{{ $company->active ? 'btn-toggle-on' : 'btn-toggle-off' }}">
                ...
                @empty
                <tr>
                    <td colspan="8" style="text-align:center; padding:2rem; color:rgba(148,163,184,.4);">
