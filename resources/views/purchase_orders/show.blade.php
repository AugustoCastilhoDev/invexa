        ...
        <div class="card card-dark-bg shadow-sm">
            ...
            <div class="card-body">
                ...
                <dl class="row mb-0" style="font-size:.875rem;">
                    ...
                    <dd class="col-7 text-white py-2 border-bottom border-secondary">{{ $purchaseOrder->created_at->format('d/m/Y') }}</dd>
                    ...
                    <dd class="col-7 text-white py-2 border-bottom border-secondary">
                        {{ $purchaseOrder->expected_date ? $purchaseOrder->expected_date->format('d/m/Y') : '&mdash;' }}
                    </dd>

                    @if($purchaseOrder->received_at)
                    <dt class="col-5 text-soft fw-normal py-2 border-bottom border-secondary">Recebido em</dt>
                    <dd class="col-7 text-white py-2 border-bottom border-secondary">{{ $purchaseOrder->received_at->format('d/m/Y') }}</dd>
                    @endif

                    <dt class="col-5 text-soft fw-normal py-2 border-bottom border-secondary">Total</dt>
                    <dd class="col-7 fw-bold text-white py-2 border-bottom border-secondary">R$ {{ number_format($purchaseOrder->total, 2, ',', '.') }}</dd>

                    @if($purchaseOrder->notes)
                    <dt class="col-5 text-soft fw-normal py-2">Observações</dt>
                    <dd class="col-7 text-soft py-2">{{ $purchaseOrder->notes }}</dd>
                    @endif
                </dl>
            </div>
        </div>
                            ...
                            @foreach($purchaseOrder->items as $item)
                                <tr style="border-color:rgba(148,163,184,.07);">
                                    ...
                                    <td class="py-3 text-center">
                                        <span class="text-success fw-semibold">{{ $item->quantity_received }}</span>
                                    </td>
                                    <td class="py-3 text-center">
                                        @if($item->pending > 0)
                                            <span class="text-warning fw-semibold">{{ $item->pending }}</span>
                                        @else
