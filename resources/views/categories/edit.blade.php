...
<div class="card dashboard-card card-dark-bg shadow-sm border-0">
    <div class="card-header card-header-dark border-bottom">
        <h4 class="mb-1 text-white">Editar Categoria</h4>
        <p class="text-soft mb-0">Atualize os dados da categoria selecionada.</p>
    </div>
    <div class="card-body">
        <form action="{{ route('categories.update', $category) }}" method="POST">
            ...
            <div class="mb-4">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="active" id="active" {{ $category->active ? 'checked' : '' }}>
                    <label class="form-check-label text-white" for="active">Categoria Ativa</label>
                </div>
                <small class="text-soft">Altere o status para ativar ou desativar este grupo.</small>
            </div>

            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary px-4">Atualizar Categoria</button>
