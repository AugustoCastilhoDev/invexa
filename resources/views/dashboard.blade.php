...
.badge-cancelada, .badge-cancelled {
...
/* ── Cabeçalhos de tabela com destaque ── */
        ...
        <div class="card kpi-card dashboard-card text-white h-100"
            ...
            <div class="card-body d-flex flex-column justify-content-between gap-2">
                ...
                <div>
                    ...
                    @php $productsTrend = $productsTrend ?? null; @endphp
                    @if (!is_null($productsTrend))
                        <span class="kpi-trend {{ $productsTrend >= 0 ? 'up' : 'down' }}">
                            <i class="bi bi-arrow-{{ $productsTrend >= 0 ? 'up' : 'down' }}-short"></i>
                            {{ $productsTrend >= 0 ? '+' : '' }}{{ number_format($productsTrend, 1, ',', '.') }}% vs anterior
                        </span>
                    @else
        ...
        <div class="card kpi-card dashboard-card text-white h-100"
            ...
            <div class="card-body d-flex flex-column justify-content-between gap-2">
                ...
                <div>
                    ...
                    @php $categoriesTrend = $categoriesTrend ?? null; @endphp
                    @if (!is_null($categoriesTrend))
                        <span class="kpi-trend {{ $categoriesTrend >= 0 ? 'up' : 'down' }}">
                            <i class="bi bi-arrow-{{ $categoriesTrend >= 0 ? 'up' : 'down' }}-short"></i>
                            {{ $categoriesTrend >= 0 ? '+' : '' }}{{ number_format($categoriesTrend, 1, ',', '.') }}% vs anterior
                        </span>
                    @else
                        <span class="kpi-trend neutral"><i class="bi bi-dash"></i> Sem comparativo</span>
                    @endif
        ...
        <div class="card kpi-card dashboard-card text-white h-100"
            ...
            <div class="card-body d-flex flex-column justify-content-between gap-2">
                ...
                <div>
                    ...
                    @php $salesTrend = $salesTrend ?? null; @endphp
                    @if (!is_null($salesTrend))
                        <span class="kpi-trend {{ $salesTrend >= 0 ? 'up' : 'down' }}">
                            <i class="bi bi-arrow-{{ $salesTrend >= 0 ? 'up' : 'down' }}-short"></i>
                            {{ $salesTrend >= 0 ? '+' : '' }}{{ number_format($salesTrend, 1, ',', '.') }}% vs anterior
                        </span>
                    @else
                        <span class="kpi-trend neutral"><i class="bi bi-dash"></i> Sem comparativo</span>
                    @endif
        ...
        <div class="card kpi-card dashboard-card text-white h-100"
            ...
            <div class="card-body d-flex flex-column justify-content-between gap-2">
                ...
                <div>
                    ...
                    <div style="font-size:.8rem; opacity:.8;">Receita total</div>
                ...
                <p class="kpi-description mb-0">Receita do período selecionado.</p>
            </div>
        </div>
    </div>
    ...
    <div class="col-12 col-xl-8">
        <div class="card dashboard-card card-dark-bg shadow-sm h-100">
            <div class="card-header card-header-dark border-bottom d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                <div>
                    <h5 class="mb-1 text-white">Tendência de vendas</h5>
                    <p class="text-soft mb-0">Receita por dia para o período selecionado.</p>
                </div>
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('dashboard', array_merge(request()->all(), ['interval' => '7d'])) }}" class="btn btn-sm {{ $interval === '7d' ? 'btn-primary' : 'btn-outline-light' }}">Últimos 7 dias</a>
        ...
        <div class="card dashboard-card card-dark-bg shadow-sm h-100">
            ...
            <div class="card-body d-flex flex-column gap-2 pt-3">
                ...
                <div class="metric-row">
                    <div class="metric-content">
                        <div>
                            <div class="metric-label">Venda de hoje</div>
                            <div class="metric-value">R$ {{ number_format($salesToday ?? 0, 2, ',', '.') }}</div>
                        </div>
                    </div>
                ...
                <div class="metric-row">
                    <div class="metric-content">
                        <div>
                            ...
                            <div class="metric-value">R$ {{ number_format($periodMaxSale ?? 0, 2, ',', '.') }}</div>
                        </div>
                    </div>
                ...
                <div class="metric-row">
                    <div class="metric-content">
                        <div>
                            <div class="metric-label">Variação receita</div>
                ...
                <div class="table-responsive">
                    <table class="table table-dark table-hover mb-0 align-middle table-dark-custom">
                        ...
                        <tbody>
                                    ...
                                    <td>
                                        @php
                                            $statusMap = [
                                                ...
                                                'paid' => ['class' => 'badge-paid', 'label' => 'Pago'],
                                                ...
                                                'cancelled' => ['class' => 'badge-cancelled', 'label' => 'Cancelada'],
                                            ];
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-soft py-4">Nenhuma venda encontrada.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @forelse($lowStockProducts as $product)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge-status badge-cancelada">
                                            {{ $product->quantity }}
                                        </span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center text-soft py-4">Nenhum produto com estoque baixo.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
...
<div class="card dashboard-card card-dark-bg shadow-sm mb-4">
    <div class="card-body">
        ...
        <div class="row g-3">
            ...
            <div class="col-12 col-md-4">
                <div class="p-3 rounded-3 bg-white bg-opacity-10 text-white">
                    <div class="text-soft small">Vendas anteriores</div>
                ...
                y: {
                    ...
                    ticks: {
                        ...
                        callback: function (value) {
                            if (value >= 1000) {
                                return 'R$ ' + (value / 1000).toLocaleString('pt-BR', { minimumFractionDigits: 0 }) + 'k';
                            }
